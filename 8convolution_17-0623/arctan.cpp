...
//char date_directory4[128]="..\\result_usa\\";
...
//Nrutilを用いたメモリの確保
	...
	for (int change_image = 0; change_image < 3; ++change_image) {
		if (change_image == 0)parameter_atan = 3;
		if (change_image == 1)parameter_atan = 10;
		if (change_image == 2)parameter_atan = 100;

		for (sd_atan = 0; sd_atan <= 50; sd_atan = sd_atan + 10) {
			int count_small = 0, count_large = 0;

			////////////////////////////入出力ディレクトリの作成(C式のため汚い)//////////////////////////////////////////////////////////////////////////////
			sprintf(inputdate_directory_atan, "%s%dk_conv_sd%d", date_directory, parameter_atan, sd_atan);
			//Input
			...
			sprintf(date_directory4, "%s%dk_atan_sd%d", date_directory, parameter_atan, sd_atan);
			...
			/////////////////////////出力ファイルを開く///////////////////////////////////////////////////////////////////////////////////
			if ((fp_arctan = fopen(math_atan1, "w")) == NULL) { cout << "入力エラー arctan.csvが開けません"; exit(1); }
			if ((fp_threshold_atan_high = fopen(math_atan5, "w")) == NULL) { cout << "入力エラー threshold_atan_high.csvが開けません"; exit(1); }
			if ((fp_atan_threshold2 = fopen(math_atan8, "w")) == NULL) { cout << "入力エラー threshold2.csvが開けません"; exit(1); }
			if ((fp_threshold_atan_low = fopen(math_atan9, "w")) == NULL) { cout << "入力エラー threshold.csvが開けません"; exit(1); }
			...
			for (i = 0; i < image_y; ++i) {
				for (j = 0; j < image_x; ++j) {						//一行ずつ計算する
					...
					if (Angle[j][i] < 0)Angle[j][i] = Angle[j][i] + 360;
