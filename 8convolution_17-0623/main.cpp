...
void main(int argc, char** argv) {
	...
	while (propety_dire.getline(propety, 256 - 1)){
		...
		if (count_property == 3)sprintf(inputfilter_directory1, propety);
		if (count_property == 4)sprintf(inputfilter_directory2, propety);
		if (count_property == 5)sprintf(inputfilter_directory3, propety);
		if (count_property == 6)sprintf(inputfilter_directory4, propety);
		if (count_property == 7)sprintf(inputfilter_directory5, propety);
		if (count_property == 8)sprintf(inputfilter_directory6, propety);
		if (count_property == 9)sprintf(inputfilter_directory7, propety);
		if (count_property == 10)sprintf(inputfilter_directory8, propety);
...
//if(Rvector_create==1)sprintf(InputImage, "..\\bmp\\255-0\\%d.bmp", filter_number);	//Rvectorを作成する際の入力画像指定.
	...
	sprintf(Filter1, inputfilter_directory1);
	sprintf(Filter2, inputfilter_directory2);
	sprintf(Filter3, inputfilter_directory3);
	sprintf(Filter4, inputfilter_directory4);
	sprintf(Filter5, inputfilter_directory5);
	sprintf(Filter6, inputfilter_directory6);
	sprintf(Filter7, inputfilter_directory7);
	sprintf(Filter8, inputfilter_directory8);
	...
	*/
	...
	for (int y = 0; y < image_y; y++) {
		for (int x = 0; x < image_x; x++) {
			...
			int G = 3 * x + 1;
			int B = 3 * x + 2;
			input_bmp[x][y] = ImputImage.at<unsigned char>(y, R)-Offset;
			//printf("%f,",input_bmp[x][y]);
		}//printf("\n");
	}
	////////////////////////////初期設定4 : 出力結果のファイル名の指定//////////////////////////////////////////////////////////////////////////////////
	...
	for(int kernel_number=0;kernel_number<8;++kernel_number){
		sprintf(name,"inputfilter_directory%d",kernel_number);
	sprintf(inputfilter_directory,inputfilter_directory1);

	read_filter(inputfilter_directory,fs,spfil1);
	convolution(image_y,image_x,fs,hfs,output1,spfil1,input_bmp,magnification);
	sprintf(Filename,Filename1);
